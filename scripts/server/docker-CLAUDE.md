...
- **Homepage** joins the `arrs` network (static IP `172.20.0.250`) so it can contact arrs-stack services by container name for its widgets. **Privileges slimmed 2026-07-02:** the Docker socket, `/hostfs`, `/mnt/*` mounts, raw disk devices, and SYS_RAWIO/SYS_ADMIN were all removed as vestigial — the docker.yaml integration was never configured, and all CPU/drive stats come from the drivetemps sidecar, fetched browser-side by `custom.js`. Homepage now mounts only its own config + icons. (The custom `homepage-smartctl` image is retained for its frontend JS patch — the Dockerfile's "drive temp" display-truncation tweak, `s.slice(0,4)`→`(0,6)`; the smartmontools layer inside it is inert without devices/caps, and the image could likely go stock after a visual check that nothing regresses.)
