...
**Naming quirk on the last 4:** because the earlier stuck-node troubleshooting wiped and recreated their tsnet identities more than once, the *old* identities (`ntfy`, `immich`, `open-webui`, `pihole`) are still registered in the Tailscale admin console as orphaned **offline** devices (visible in `tailscale status` as "offline, last seen ..."). When the certs finally issued, tsdproxy's tsnet nodes couldn't reuse the bare names (already taken by the offline duplicates) and landed on suffixed names instead: `ntfy-1`, `immich-1`, `open-webui-1`, `pihole-1`. Homepage's hrefs for these 4 point at the `-1` hostnames to match. To reclaim the clean names: delete the 4 offline duplicate devices from `https://login.tailscale.com/admin/machines` (manual step — no CLI/API access is configured for remote device deletion), then wipe+recreate those 4 tsdproxy nodes once more (safe now that the rate limit isn't a factor).
