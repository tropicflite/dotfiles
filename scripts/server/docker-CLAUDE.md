...
- **watchtower** (`~/docker/watchtower/`) runs `WATCHTOWER_MONITOR_ONLY: "true"` (changed 2026-07-01) — it checks all non-excluded containers nightly at 4am and sends an ntfy notification when updates are available, but **never applies them automatically**. Most services here run `:latest` or an unpinned moving tag, so blind auto-apply meant a breaking upstream change could already be deployed before anyone noticed; now it's review-then-manual (`docker compose pull && up -d` per stack) instead. `WATCHTOWER_ROLLING_RESTART` is incompatible with monitor-only and was removed. `WATCHTOWER_NOTIFICATIONS_LEVEL` must stay at `info`, not `warn` — "update available" logs at info, so `warn` would silently swallow the very notifications this mode exists to produce. `watchtower/.env`'s `WATCHTOWER_NOTIFICATION_URL` points at `100.65.250.53:2586` (was broken pointing at `localhost` when ntfy had no loopback binding; ntfy regained a loopback binding 2026-07-02, so either address works now). **Exclusion policy (revised 2026-07-02):** under monitor-only, `com.centurylinklabs.watchtower.enable=false` suppresses *notifications*, not updates — so it now marks only images where "update available" is meaningless: the two locally-built images (homepage, drivetemps) and Immich's digest-pinned postgres/redis. immich-server, immich-machine-learning, and tsdproxy had their exclusions removed so their updates get noticed (label change takes effect when those containers are next recreated).
